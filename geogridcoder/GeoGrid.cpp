...
extern "C" {
PG_MODULE_MAGIC;
PG_FUNCTION_INFO_V1(gsg_geohash);
PG_FUNCTION_INFO_V1(gsg_boxgeohash);
...
Datum gsg_geohash(PG_FUNCTION_ARGS)
{
	...
	rect.left = box->xmin;
	rect.right = box->xmax;
	rect.bottom = box->ymin;
	rect.top = box->ymax;
	//栅格化
	HashRasterize((unsigned char *)wkb->data,
		      wkb_len,
		      nullptr, // srs
		      rect,
		      level,
...
{
	varlena *buf = PG_GETARG_VARLENA_P(0);
	int16_t has_z = *reinterpret_cast<uint16_t *>(buf->vl_dat);
...
{
	varlena *buf = PG_GETARG_VARLENA_P(0);
	int16_t level = *reinterpret_cast<uint16_t *>(buf->vl_dat + 2);
