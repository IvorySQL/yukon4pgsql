    ...
    const unsigned int level, RawHashData &data)
{
    ...
    /* 计算 geometry 的包围网格 */
    subextent.MinX = floor((src_env.MinX - global_extent.left) / _scale[0]) * _scale[0] + global_extent.left;
    subextent.MinY = floor((src_env.MinY - global_extent.bottom) / _scale[1]) * _scale[1] + global_extent.bottom;
    subextent.MaxX = ceil((src_env.MaxX - global_extent.left) / _scale[0]) * _scale[0] + global_extent.left;
    subextent.MaxY = ceil((src_env.MaxY - global_extent.bottom) / _scale[1]) * _scale[1] + global_extent.bottom;
