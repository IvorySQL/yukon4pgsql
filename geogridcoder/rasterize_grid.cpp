	      			...
	      			uint32_t level, uint32_t geom_type, vector<GridPos> &results, GBOX *src_box)
{
	...
	if (10 <= level && level <= 12)
	{
		...
		RegularRasterize(_drv, src_geom, src_env, subextent, options, 9, UNIFIED, geom_type, record, degree_record);
		//计算每个度级网格里的不规则栅格
		int minutes_level = level - 9;
		RegularRasterize(_drv, src_geom, src_env, subextent, options, minutes_level, MINUTE, geom_type, degree_record, results);
	}
	else if (16 <= level && level <= 18)
	{
		//如果精度等级在16-18，为不规则栅格，其上层为规则栅格，先计算分级的网格并存储起来
		RegularRasterize(_drv, src_geom, src_env, subextent, options, 15, UNIFIED, geom_type, record, minute_record);
		//计算每个分级网格里的不规则栅格
		int seconds_level = level - 15;
		RegularRasterize(_drv, src_geom, src_env, subextent, options, seconds_level, SECOND, geom_type, minute_record, results);
	}
	else
		RegularRasterize(_drv, src_geom, src_env, subextent, options, level, UNIFIED, geom_type, record, results);
		...
		for (iYBlock = 0; iYBlock < nYBlocks; iYBlock++)
		{
			for (iXBlock = 0; iXBlock < nXBlocks; iXBlock++)
			{
                ...
                cplerr = GDALRasterIO(
                    ...
                    x, y,
                    nXValid, nYValid,
                    &grid_data.val[0] + valueslen * iYBlock, nXValid, nYValid,
                    gdpixtype,
                    0, 0);
				if (cplerr != CE_None)
				{
