...
bool
geomodel_isvalid(GSERIALIZED *gs)
{
...
bool
geomodel_reset_flags(GSERIALIZED *gs)
{
...
GSERIALIZED *
wrap_header_geomodeldata(char *pData, int nLen, YkBoundingBox &bbox)
{
	int size = 0;
	// size,srid,flags
	size += 8;
 ...
 */
bool
gmserialized_get_bbox(GSERIALIZED *gms, BOX3D *bbox)
{
 ...
 */
bool
gmserialized_set_bbox(GSERIALIZED *gms, YkBoundingBox &bbox)
{
	//设置包围盒
...
Datum make_skeleton_from_tin(PG_FUNCTION_ARGS)
{
	...
	int32_t srid = gserialized_get_srid(input);
	//补充 ==> 准入条件
	//  if(srid != 3857)
	//  {
	//    return 0;
