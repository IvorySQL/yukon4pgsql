 ...
 */
uint32
gmserialized_typmod_in(ArrayType *arr)
{
	uint32 typmod = 0;
	Datum *elem_values;
	int n = 0;
...
int
geomodel_getSRID(GSERIALIZED *s)
{
	int32_t srid = 0;
...
void
geomodel_setSRID(GSERIALIZED *s, int32_t srid)
{
	srid = clamp_srid(srid);
...
Datum model_elem_enforce_typmod(PG_FUNCTION_ARGS)
{
	...
	if (pElement->GetType() == etSkeleton)
	{
		...
		if ((bExact && (typmod_type == MESHTYPE)) || (!bExact && (typmod_type == SURFACETYPE)))
		{
			ereport(ERROR,
				(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
				 errmsg("SKELETON type (%s) does not match column type (%s)",
					geomodel_type_name(bExact ? SURFACETYPE : MESHTYPE),
					geomodel_type_name(typmod_type))));
			delete pElement;
			PG_RETURN_NULL();
		}
	}
