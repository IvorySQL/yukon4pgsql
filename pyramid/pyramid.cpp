...
Datum buildpyramid(PG_FUNCTION_ARGS);
...
Datum
yukon_pyramid_version(PG_FUNCTION_ARGS)
{
	char src[100] = {0};
...
buildpyramid(PG_FUNCTION_ARGS)
{
	...
	if (ret > 0 && SPI_tuptable != NULL)
	{
		...
		for (size_t i = 0; i < tuptable->numvals; i++)
		{
	...
	else if (geomtype == "POINT" || geomtype == "MULTIPOINT")
	{
		...
		snprintf(query, QUERYSIZE, "ANALYZE \"%s\".\"%s\"", schemaname, tablename);
		ret = SPI_exec(query, 0);

		if (ret <= 0)
		{
			elog(ERROR, "%s:analyze error!", __FUNCTION__);
		}
 ...
 */
Datum
updatePyramid(PG_FUNCTION_ARGS)
{
	// 1. 开始检查参数
